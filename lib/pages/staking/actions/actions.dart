...
class StakedInfo {
  ...
  StakedInfo(String _address, String _votes, String _interests, String _freeze,
      List<BondedChunksData> _unbondedChunks) {
    address = _address;
    votes = _votes;
...
class _StakingActions extends State<StakingActions>
  ...
  Future<void> _updateStakingTxs() async {
    ...
    await widget.plugin.service.staking
        .queryNominations(widget.keyring.current.address);
    if (mounted) {
      setState(() {
        _loading = false;
      });
  ...
  void _showActions(StakedInfo info) {
    ...
    final validator = widget.plugin.store.staking.validatorsInfo
        .where((val) => val.accountId == info.address)
        ?.first;
    ...
    showCupertinoModalPopup(
      ...
      builder: (BuildContext context) => CupertinoActionSheet(
        actions: ValidatorSortOptions.values
            .where((i) =>
                Fmt.balanceInt(info.freeze) > BigInt.zero ? true : i.index != 4)
            .map((i) => CupertinoActionSheetAction(
                  child: Text(dicStaking[
                      'mystaking.action.' + i.toString().split('.')[1]]),
                  onPressed: () {
                    ...
                    switch (i.index) {
                      case 0:
                        ...
                        break;
                      case 1:
                        Navigator.of(context).pushNamed(ClaimPageWrapper.route,
                            arguments: validator);
                        break;
                      case 2:
                        Navigator.of(context).pushNamed(
                            UnboundPageWrapper.route,
                            arguments: UnboundArgData(
                                validator,
                                Fmt.priceFloorBigInt(
                                    Fmt.balanceInt(info.votes), 8,
                                    lengthMax: 4)));
                        break;
                      case 3:
                        Navigator.of(context).pushNamed(RebondPageWrapper.route,
                            arguments: UnboundArgData(
                                validator,
                                Fmt.priceFloorBigInt(
                                    Fmt.balanceInt(info.votes), 8,
                      ...
                      case 4:
                        Navigator.of(context).pushNamed(
                            UnfreezePageWrapper.route,
                            arguments: UnfreezeArgData(
                                validator, info.unbondedChunks));
                        break;
                      default:
                        break;
                    }
  ...
  List<Widget> _buildMyStakedValidatorsList() {
    List<NominationData> validNominations =
        widget.plugin.store.staking.validNominations;
    List<UserInterestData> userInterests =
        widget.plugin.store.staking.userInterests;
    ...
    res.add(Padding(
        ...
        child:
          ...
          Text(dicStaking['mystaking.label'],
    ...
    if (currentAccount.isNotEmpty) {
      validNominations.forEach((nmn) {
        ...
        if (nmn.account == currentAccount) {
          BigInt interest = userInterests.length > 0
              ? BigInt.parse(userInterests[0]
                  .interests
                  .firstWhere((i) => i.validator == nmn.validatorId)
                  ?.interest)
              : BigInt.zero;
          txs.add(StakedInfo(nmn.validatorId, nmn.nomination.toString(),
              interest.toString(), chunks.toString(), nmn.unbondedChunks));
        }
      });
    }
      ...
      return Container(
        ...
        child: ListTile(
          ...
          subtitle:
            ...
            Text(
                '${dicStaking['mystaking.votes']}: ${Fmt.priceFloorBigInt(Fmt.balanceInt(i.votes.toString()), 8, lengthMax: 4)}',
                style: TextStyle(color: Colors.green)),
            Text(
                '${dicStaking['mystaking.freeze']}: ${Fmt.priceFloorBigInt(Fmt.balanceInt(i.freeze.toString()), 8, lengthMax: 4)}'),
          ]),
          trailing: Column(
            crossAxisAlignment: CrossAxisAlignment.end,
            children: [
              Text(dicStaking['mystaking.interests'],
                  style: TextStyle(color: Colors.red)),
              Text(
                  Fmt.priceFloorBigInt(
                      Fmt.balanceInt(i.interests.toString()), 8,
                      lengthMax: 4),
                  style: TextStyle(color: Colors.red))
            ],
          ),
      ...
      builder: (_) {
        List<Widget> list = <Widget>[
          TopCard(
              ...
              widget.plugin.store.staking.validNominations,
              widget.plugin.store.staking.nominationLoading ||
                  widget.plugin.sdk.api.connectedNode == null,
              widget.keyring.current.address),
        ];
