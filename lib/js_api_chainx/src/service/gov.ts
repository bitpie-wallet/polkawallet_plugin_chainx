...
async function getReferendumVoteConvictions(api: ApiPromise) {
  const enact = (((((<any>PERIODS)[api.genesisHash.toHex()] || api.consts.democracy.enactmentPeriod).toNumber() * api.consts.timestamp.minimumPeriod.toNumber()) / 1000) * 2) / SEC_DAY
